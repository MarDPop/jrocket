 ...
 * @author mpopescu
 */
public class DynamicsBallistic extends Dynamics {
    ...
    public DynamicsBallistic(int nStates, double BC_super, double BC_sub){
        super(nStates);
        if(nStates < 6) {
    ...
    public double[] calc(double[] x, double t) {
        ...
        if ((g-Earth.R_EARTH_MEAN) < 0) {
            ...
            d = atm.getDensity(g-Earth.R_EARTH_MEAN)*d/BC;
            g = -Earth.STANDARD_GRAVITATIONAL_PARAMETER/(g*g*g);
            dx[3] = x[0]*g + v[0]*d;
            dx[4] = x[1]*g + v[1]*d;
            dx[5] = x[2]*g + v[2]*d;
        }
